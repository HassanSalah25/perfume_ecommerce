<div class="de__pcard text-center">
    <div class="atr__images">
        @if (discount_in_percentage($product) > 0)
            <div class="atr__promo">-{{ discount_in_percentage($product) }}%</div>
        @endif
    ...
    @if ($product->choice_options && count(json_decode($product->choice_options)))
        <div class="atr__opt mt-2">
            @foreach (json_decode($product->choice_options) as $opt)
                @foreach ($opt->values as $i => $value)
                    @if ($i < 3)
                        <div class="atr__opt-{{ $i+1 }}{{ $i === 0 ? ' active' : '' }}" data-value="{{ $value }}">
                            {{ $value }}
                        </div>
                    @endif
                @endforeach
